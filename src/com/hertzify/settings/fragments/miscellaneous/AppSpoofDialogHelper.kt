...
class AppSpoofDialogHelper(
    ...
    private fun createHeaderView(titleText: String): RelativeLayout {
    ...
    fun showAddAppDialog() {
        backStack.clear()
        val layout = MaxHeightLinearLayout(context, 0.85f).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(0, px(16), 0, 0)
            clipChildren = true
        }

        layout.addView(createHeaderView(context.getString(R.string.spoof_ap_select_app)))
    ...
    fun showManageProfilesDialog(
        ...
        if (!isPickerMode) {
            ...
            val btnAdd = createPillButton(context.getString(R.string.spoof_ap_add_profile), accentBg, accentColor, true) {
                backStack.push { showManageProfilesDialog() }
    ...
    private class MaxHeightLinearLayout @JvmOverloads constructor(
        context: Context,
        private val maxHeightRatio: Float = 0.85f
